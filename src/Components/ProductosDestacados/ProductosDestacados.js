...
const ProductosDestacados = () => {
  ...
  const productosAleatorios = productos.sort(function () {
    return Math.random() - 0.5;
  });

  const productosFiltrados = productosAleatorios.filter(
    (producto) => (productos.length = 4)
  );
  ...
  return (
    <div className="productos-destacados">
        ...
        {productosFiltrados.map((producto) => (
          <div className="card-producto bg-light m-1 border rounded-3">
            <img
              ...
              className="card-img-top fs-4 p-1"
              alt={producto.nombre}
            />
            <div className="card-body">
              <h3 className="card-title title-producto d-flex justify-content-center align-items-center">
                {producto.nombre}
              </h3>
      ...
      <a href="/productos" className="d-block">
        <button className="btn btn-verproductos m-4 p-3">
